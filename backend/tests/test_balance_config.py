...
class BalanceConfigTests(unittest.TestCase):
    def test_default_balance_config_bundle_parses_into_new_three_phase_structure(self) -> None:
        ...
        self.assertEqual(config.production.goods["steel"].route_id, "mechanized")
        self.assertEqual(config.production.goods["grain"].unit_budget_cost, 1)
        self.assertEqual(config.production.goods["grain"].demand_threshold, 5)
        self.assertEqual(config.production.goods["grain"].price_floor, 2)
        self.assertEqual(config.production.goods["grain"].price_ceiling, 12)
        self.assertEqual(config.production.upgrade_costs["electrified"], 24)
        self.assertEqual(config.production.new_factory_costs["handicraft"], 10)
