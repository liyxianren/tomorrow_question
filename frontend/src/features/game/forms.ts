...
const INVALID_SUBMISSION_MESSAGE_PATTERNS: ReadonlyArray<{
  ...
  { match: /^Unknown colonization region: (\S+)/i, translate: (m) => i18n.t("game:submit.errorUnknownColonizationRegion", "未知殖民地区：{{region}}。", { region: m[1] }) },
  { match: /^Duplicate colonization region: (\S+)/i, translate: (m) => i18n.t("game:submit.errorDuplicateColonizationRegion", "殖民地区 {{region}} 重复提交。", { region: m[1] }) },
  { match: /^Cannot colonize region (\S+)/i, translate: (m) => i18n.t("game:submit.errorCannotColonizeRegion", "地区 {{region}} 当前不可殖民。", { region: m[1] }) },
  { match: /^Not enough army for colonization/i, translate: () => i18n.t("game:submit.errorColonizationArmyInsufficient", "陆军不足，无法殖民。") },
  ...
  { match: /^Overseas competition region (\S+) route is blocked/i, translate: (m) => i18n.t("game:submit.errorCompetitionRouteBlocked", "海外争夺区域 {{region}} 地区被封锁。", { region: m[1] }) },
